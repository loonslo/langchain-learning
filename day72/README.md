...
先还原并跑通 Day71，再开始今天。Day72 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。
...
1. 打开 `src/customer_support/capacity.py`：搜索并跟踪：`CapacityReport`、`report`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_capacity.py`：搜索并跟踪：`test_capacity_fails_on_latency_or_errors`。写出输入、输出、调用方和失败分支。
