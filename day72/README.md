...
> 今天解决：单次响应快不能说明容量。
>
> 第一性原则：p95 与错误率共同定义可接受负载。

## 1. 与 Day71 的文件衔接

先还原并跑通 Day71，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。
...
| `tests/test_capacity.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `tests/test_release_readiness.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/readiness.py` | 修改旧文件（上一版 Day70） | 启动检查 |
...
运行 `python tools/day_change_report.py 72` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
配置检查 + 压测样本 → release_readiness → 发布判定
```
...
src/customer_support/capacity.py  # 本日变更
...
src/customer_support/readiness.py  # 本日变更
...
tests/test_release_readiness.py  # 本日变更
...
1. 打开 `src/customer_support/readiness.py`：搜索并跟踪：`readiness`、`ensure_ready`、`release_readiness`。写出输入、输出、调用方和失败分支。
2. 打开 `src/customer_support/capacity.py`：搜索并跟踪：`CapacityReport`、`report`。写出输入、输出、调用方和失败分支。
3. 打开 `tests/test_capacity.py`：搜索并跟踪：`test_capacity_fails_on_latency_or_errors`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_release_readiness.py`：搜索并跟踪：`test_capacity_result_enters_the_release_readiness_gate`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 72
cd .build\day72\customer-support
...
假样本不代表真实容量。把实际运行结果写入 `workbook.md`。
