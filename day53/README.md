...
> 今天解决：手工试问无法阻止质量回归。
>
> 第一性原则：评测集是可执行的产品需求，答案和引用要分开判断。

## 1. 与前一天的关系

先还原并跑通 Day52，再开始今天。Day53 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。

## 2. 今天新增或修改

| 项目相对路径 | 变更 | 职责 |
|---|---|---|
| `data/eval_cases.json` | 新增 | 项目真实输入资料或评测数据 |
| `src/customer_support/evaluation.py` | 新增 | 离线用例与分层判断 |
| `tests/test_evaluation.py` | 新增 | 验证今天新增能力的正向和失败路径 |

## 3. Day53 结束后的完整项目结构

```text
.env.example  # 继承 Day51
data/eval_cases.json  # 本日变更
...
1. 打开 `src/customer_support/evaluation.py`：搜索并跟踪：`Assistant`、`Assistant.ask`、`EvalCase`、`load_cases`、`evaluate`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_evaluation.py`：搜索并跟踪：`FakeAssistant`、`FakeAssistant.ask`、`test_correct_text_with_wrong_citation_still_fails`。写出输入、输出、调用方和失败分支。
...
关键词判断不等于完整语义评测。把实际运行结果写入 `workbook.md`。
