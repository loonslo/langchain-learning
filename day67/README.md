...
> 今天解决：线上慢或错时没有证据定位。
>
> 第一性原则：成功失败都要留下不含原始问题的 trace。

## 1. 与 Day66 的文件衔接

先还原并跑通 Day66，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。
...
| `tests/test_observability.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/bootstrap.py` | 修改旧文件（上一版 Day66） | 创建真实依赖并接入正式主链 |
| `tests/test_application.py` | 修改旧文件（上一版 Day66） | 验证今天新增能力的正向和失败路径 |
...
运行 `python tools/day_change_report.py 67` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
API/CLI → ObservedApplication → 安全/缓存/业务链 → Trace
```
...
src/customer_support/observability.py  # 本日变更
...
tests/test_observability.py  # 本日变更
...
2. 打开 `src/customer_support/observability.py`：搜索并跟踪：`Trace`、`Recorder`、`Recorder.measure`、`ObservedApplication`、`ObservedApplication.handle`、`ObservedApplication.ask`。写出输入、输出、调用方和失败分支。
3. 打开 `tests/test_application.py`：搜索并跟踪：`Product`、`Product.handle`、`test_real_product_call_is_observed_without_recording_question`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_observability.py`：搜索并跟踪：`test_success_and_failure_are_observed_without_question`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 67
cd .build\day67\customer-support
...
内存记录器不是生产监控平台。把实际运行结果写入 `workbook.md`。
