...
> 今天解决：单一 FAQ 无法维护多类政策。
>
> 第一性原则：稳定来源与 chunk_id 是引用、更新和排错的基础。

## 1. 与前一天的关系

先还原并跑通 Day51，再开始今天。Day52 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。

## 2. 今天新增或修改

| 项目相对路径 | 变更 | 职责 |
|---|---|---|
| `data/knowledge/refund.md` | 新增 | 项目真实输入资料或评测数据 |
| `data/knowledge/shipping.md` | 新增 | 项目真实输入资料或评测数据 |
| `src/customer_support/ingestion.py` | 新增 | 多文档加载和稳定 ID |
| `tests/test_ingestion.py` | 新增 | 验证今天新增能力的正向和失败路径 |
...
data/knowledge/refund.md  # 本日变更
data/knowledge/shipping.md  # 本日变更
...
src/customer_support/ingestion.py  # 本日变更
src/customer_support/knowledge.py  # 继承 Day51
src/customer_support/settings.py  # 继承 Day51
tests/test_assistant.py  # 继承 Day51
tests/test_ingestion.py  # 本日变更
tests/test_knowledge.py  # 继承 Day51
```

## 4. 按顺序阅读和动手

1. 打开 `src/customer_support/ingestion.py`：搜索并跟踪：`ingest_directory`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_ingestion.py`：搜索并跟踪：`test_multi_document_chunks_have_stable_traceable_ids`。写出输入、输出、调用方和失败分支。
...
尚未证明检索质量。把实际运行结果写入 `workbook.md`。
