...
> 今天解决：能在作者机器运行不代表可交付。
>
> 第一性原则：镜像不带密钥、非 root 运行且启动前检查依赖。

## 1. 与 Day69 的文件衔接

先还原并跑通 Day69，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。

### 今天新增、修改或删除

| 项目相对路径 | 状态 | 为什么今天要看 |
|---|---|---|
| `.dockerignore` | 新增 | 项目配置、文档或交付证据 |
| `Dockerfile` | 新增 | 可复现运行镜像 |
| `src/customer_support/readiness.py` | 新增 | 启动检查 |
| `tests/test_readiness.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/bootstrap.py` | 修改旧文件（上一版 Day68） | 创建真实依赖并接入正式主链 |
...
运行 `python tools/day_change_report.py 70` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
容器/CLI/API 启动 → bootstrap.build_application → ensure_ready → 构建依赖
```
...
2. 打开 `src/customer_support/readiness.py`：搜索并跟踪：`readiness`、`ensure_ready`。写出输入、输出、调用方和失败分支。
3. 打开 `tests/test_readiness.py`：搜索并跟踪：`test_missing_runtime_requirements_block_readiness`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 70
cd .build\day70\customer-support
...
尚未发布真实 staging。把实际运行结果写入 `workbook.md`。
