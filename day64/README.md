...
先还原并跑通 Day63，再开始今天。Day64 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。
...
1. 打开 `src/customer_support/sync.py`：搜索并跟踪：`SyncPlan`、`scan`、`plan`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_sync.py`：搜索并跟踪：`test_sync_distinguishes_change_and_delete`。写出输入、输出、调用方和失败分支。
