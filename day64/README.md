...
> 今天解决：全量重建昂贵且删除资料会残留。
>
> 第一性原则：内容哈希决定 upsert、delete 与跳过。

## 1. 与 Day63 的文件衔接

先还原并跑通 Day63，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。
...
| `tests/test_sync.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/api.py` | 修改旧文件（上一版 Day63） | HTTP 契约 |
| `src/customer_support/runtime.py` | 修改旧文件（上一版 Day63） | 真实依赖与最终 API 组合入口 |
| `tests/test_api.py` | 修改旧文件（上一版 Day63） | 验证今天新增能力的正向和失败路径 |
...
运行 `python tools/day_change_report.py 64` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
HTTP /knowledge/sync-plan → SyncingApplication → scan/plan → knowledge_path
```
...
1. 打开 `src/customer_support/api.py`：搜索并跟踪：`ChatRequest`、`ChatResponse`、`SyncRequest`、`create_app`。写出输入、输出、调用方和失败分支。
2. 打开 `src/customer_support/runtime.py`：搜索并跟踪：`create_runtime_api`。写出输入、输出、调用方和失败分支。
3. 打开 `src/customer_support/sync.py`：搜索并跟踪：`SyncPlan`、`scan`、`plan`、`SyncingApplication`、`SyncingApplication.handle`、`SyncingApplication.ask`、`SyncingApplication.plan_sync`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_api.py`：搜索并跟踪：`Product`、`Product.plan_sync`、`test_incremental_sync_plan_is_reachable_from_the_product_api`。写出输入、输出、调用方和失败分支。
5. 打开 `tests/test_sync.py`：搜索并跟踪：`test_sync_distinguishes_change_and_delete`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 64
cd .build\day64\customer-support
...
执行向量事务和失败恢复仍待实现。把实际运行结果写入 `workbook.md`。
