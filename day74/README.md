...
先还原并跑通 Day73，再开始今天。Day74 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。
...
1. 打开 `src/customer_support/providers.py`：搜索并跟踪：`Provider`、`Provider.answer`、`TemporaryProviderError`、`answer_with_fallback`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_providers.py`：搜索并跟踪：`P`、`P.answer`、`test_temporary_failure_uses_fallback`。写出输入、输出、调用方和失败分支。
