...
先还原并跑通 Day64，再开始今天。Day65 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。
...
1. 打开 `src/customer_support/security.py`：搜索并跟踪：`suspicious`、`filter_documents`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_security.py`：搜索并跟踪：`test_injection_in_question_or_document_is_detected`。写出输入、输出、调用方和失败分支。
