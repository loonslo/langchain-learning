...
> 今天解决：用户和知识文档都可能携带恶意指令。
>
> 第一性原则：不可信内容要检查、隔离并审计。

## 1. 与 Day64 的文件衔接

先还原并跑通 Day64，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。
...
| `tests/test_security.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/bootstrap.py` | 修改旧文件（上一版 Day62） | 创建真实依赖并接入正式主链 |
| `src/customer_support/workflow.py` | 修改旧文件（上一版 Day56） | LangGraph 状态和分支 |
| `tests/test_application.py` | 修改旧文件（上一版 Day62） | 验证今天新增能力的正向和失败路径 |
...
运行 `python tools/day_change_report.py 65` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
API/CLI → SecuredApplication + 文档过滤 → workflow → assistant
```
...
src/customer_support/security.py  # 本日变更
...
src/customer_support/workflow.py  # 本日变更
...
2. 打开 `src/customer_support/workflow.py`：搜索并跟踪：`SupportState`、`build_graph`、`WorkflowAssistant`、`WorkflowAssistant._generate`、`WorkflowAssistant.ask`。写出输入、输出、调用方和失败分支。
3. 打开 `src/customer_support/security.py`：搜索并跟踪：`suspicious`、`filter_documents`、`SecuredApplication`、`SecuredApplication.handle`、`SecuredApplication.ask`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_application.py`：搜索并跟踪：`Product`、`Product.handle`、`test_injection_is_blocked_before_the_product_chain_runs`。写出输入、输出、调用方和失败分支。
5. 打开 `tests/test_security.py`：搜索并跟踪：`test_injection_in_question_or_document_is_detected`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 65
cd .build\day65\customer-support
...
关键词规则会漏报和误报。把实际运行结果写入 `workbook.md`。
