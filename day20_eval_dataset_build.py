"""
Day 20 · 造评测集（上）：定 schema + 写第一批样本
==========================================================
测试工程师转 AI 应用开发  ★护城河★

Day18/19 已经会"算指标"了，但评测集只有示范几条。这节正式把评测集当一个
"数据资产"来造：先定一个稳定的 schema（结构固定，后面 day18/19/RAGAS/pytest
都直接吃它），再写第一批 15+ 条——本节聚焦两类最基础的题：
  - 事实问答：答案就在某一句里，考"检索得准 + 答得对"
  - 跨段落问答：答案要拼好几处，考"召回全 + 会整合"
拒答题和引用准确性放 Day21 补齐，合并后 25 条（目标可扩到 30-50）。
...
注：keywords 字段是给 day18"关键词命中率"这种零成本指标用的（必须出现的核心词）；
...
# keywords    答案里必须出现的核心词（给 day18 关键词命中率用；拒答题留空 []）
# expect_source  应该命中的来源/章节（检索命中率用它判断召回对不对）
# type        题型：fact(事实) / multi_hop(跨段落) / refuse(拒答，Day21 加) / citation(引用)
# should_refuse  文档里没有、应当拒答 → True（Day21 大量补）
...
if __name__ == "__main__":
    ...
    print("✅ schema 校验通过。Day21 继续补拒答 + 引用题，合并到 25 条后接 RAGAS。")

# ----------------------------------------------------------
# 小结：
# - 评测集 = RAG 的回归用例库，schema 一旦定好就别乱改（下游全靠它）。
# - 存 JSON 而不是写死在代码里：能版本管理、能被 day18/19/RAGAS/pytest 共用。
