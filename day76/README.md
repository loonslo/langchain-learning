...
先还原并跑通 Day75，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。

### 今天新增、修改或删除

| 项目相对路径 | 状态 | 为什么今天要看 |
|---|---|---|
| `src/customer_support/api.py` | 修改旧文件（上一版 Day64） | HTTP 契约 |
| `src/customer_support/application.py` | 修改旧文件（上一版 Day62） | 统一业务编排 |
| `src/customer_support/runtime.py` | 修改旧文件（上一版 Day75） | 真实依赖与最终 API 组合入口 |
| `tests/test_api.py` | 修改旧文件（上一版 Day64） | 验证今天新增能力的正向和失败路径 |
| `tests/test_application.py` | 修改旧文件（上一版 Day68） | 验证今天新增能力的正向和失败路径 |
...
运行 `python tools/day_change_report.py 76` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
runtime → 认证 API → 同步/反馈/统一 application → 安全、缓存、工具、工单
```
...
3. 打开 `src/customer_support/runtime.py`：搜索并跟踪：`create_runtime_api`、`backup_threads`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_api.py`：搜索并跟踪：`Product`、`Product.handle`、`Product.plan_sync`、`test_final_api_rejects_missing_token_and_uses_signed_identity`。写出输入、输出、调用方和失败分支。
5. 打开 `tests/test_application.py`：搜索并跟踪：`Assistant`、`Assistant.ask`、`test_final_core_keeps_order_isolation_and_idempotent_escalation`。写出输入、输出、调用方和失败分支。
