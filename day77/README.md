...
先还原并跑通 Day76，再开始今天。Day77 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。
...
src/customer_support/runtime.py  # 继承 Day76
...
1. 打开 `src/customer_support/evidence.py`：搜索并跟踪：`missing_evidence`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_evidence.py`：搜索并跟踪：`test_missing_claim_evidence_is_reported`。写出输入、输出、调用方和失败分支。
