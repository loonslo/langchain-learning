...
> 今天解决：功能很多却无法清楚说明问题和证据。
>
> 第一性原则：简历和面试只陈述仓库可验证的结果。

## 1. 与 Day76 的文件衔接

先还原并跑通 Day76，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。

### 今天新增、修改或删除

| 项目相对路径 | 状态 | 为什么今天要看 |
|---|---|---|
| `docs/PROJECT_STORY.md` | 新增 | 项目配置、文档或交付证据 |
| `src/customer_support/evidence.py` | 新增 | 证据核验 |
| `tests/test_evidence.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `tests/test_runtime_evidence.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/runtime.py` | 修改旧文件（上一版 Day76） | 真实依赖与最终 API 组合入口 |

### 关键继承文件

| 项目相对路径 | 状态 | 在今天链路中的作用 |
|---|---|---|
| `src/customer_support/api.py` | 继承 Day76，今天仍被变更代码调用 | HTTP 契约 |
...
运行 `python tools/day_change_report.py 77` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
项目陈述 → runtime.verify_project_evidence → 仓库真实文件
```
...
src/customer_support/evidence.py  # 本日变更
...
tests/test_runtime_evidence.py  # 本日变更
...
1. 打开 `src/customer_support/runtime.py`：搜索并跟踪：`create_runtime_api`、`backup_threads`、`verify_project_evidence`。写出输入、输出、调用方和失败分支。
2. 打开 `src/customer_support/evidence.py`：搜索并跟踪：`missing_evidence`。写出输入、输出、调用方和失败分支。
3. 打开 `tests/test_evidence.py`：搜索并跟踪：`test_missing_claim_evidence_is_reported`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_runtime_evidence.py`：搜索并跟踪：`test_runtime_evidence_gate_rejects_an_unverifiable_claim`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 77
cd .build\day77\customer-support
...
不夸大未做的生产验证。把实际运行结果写入 `workbook.md`。
