...
先还原并跑通 Day55，再开始今天。Day56 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。

## 2. 今天新增或修改

| 项目相对路径 | 变更 | 职责 |
|---|---|---|
| `pyproject.toml` | 修改（上一版 Day51） | 截至今天的完整依赖与测试配置 |
...
1. 打开 `src/customer_support/workflow.py`：搜索并跟踪：`SupportState`、`build_graph`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_workflow.py`：搜索并跟踪：`test_graph_stops_invalid_and_refuses_without_evidence`。写出输入、输出、调用方和失败分支。
