# Day56 · LangGraph 显式控制流

> 今天解决：校验、检索、拒答和生成分支开始变复杂。
>
> 第一性原则：只有出现状态与分支时才引入图。

## 1. 与 Day55 的文件衔接

先还原并跑通 Day55，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。
...
| `tests/test_workflow.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `pyproject.toml` | 修改旧文件（上一版 Day51） | 截至今天的完整依赖与测试配置 |
| `src/customer_support/bootstrap.py` | 修改旧文件（上一版 Day55） | 创建真实依赖并接入正式主链 |

### 关键继承文件

| 项目相对路径 | 状态 | 在今天链路中的作用 |
|---|---|---|
| `src/customer_support/application.py` | 继承 Day55，今天仍被变更代码调用 | 统一业务编排 |
...
运行 `python tools/day_change_report.py 56` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
app → bootstrap → SupportApplication → WorkflowAssistant/LangGraph → assistant
```
...
src/customer_support/application.py  # 继承 Day55
...
tests/test_application.py  # 继承 Day55
...
3. 打开 `tests/test_workflow.py`：搜索并跟踪：`test_graph_stops_invalid_and_refuses_without_evidence`、`test_workflow_adapter_keeps_the_product_ask_contract`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 56
cd .build\day56\customer-support
...
用了 LangGraph 不等于自主 Agent。把实际运行结果写入 `workbook.md`。
