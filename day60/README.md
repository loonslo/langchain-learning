...
> 今天解决：内存会话在重启后丢失。
>
> 第一性原则：租户、用户、线程共同构成读取边界。

## 1. 与 Day59 的文件衔接

先还原并跑通 Day59，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。
...
| `tests/test_thread_store.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/bootstrap.py` | 修改旧文件（上一版 Day59） | 创建真实依赖并接入正式主链 |
| `src/customer_support/conversation.py` | 修改旧文件（上一版 Day55） | 会话历史与追问改写 |
| `src/customer_support/settings.py` | 修改旧文件（上一版 Day53） | 截至今天的运行路径与环境配置 |
| `tests/test_application.py` | 修改旧文件（上一版 Day59） | 验证今天新增能力的正向和失败路径 |
...
运行 `python tools/day_change_report.py 60` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
app/API → application → PersistentHistory → SQLiteThreadStore
```
...
src/customer_support/settings.py  # 本日变更
...
tests/test_thread_store.py  # 本日变更
...
1. 打开 `src/customer_support/bootstrap.py`：搜索并跟踪：`build_embeddings`、`build_chat_model`、`build_assistant`、`build_application`。写出输入、输出、调用方和失败分支。
2. 打开 `src/customer_support/conversation.py`：搜索并跟踪：`Turn`、`History`、`History.add`、`History.get`、`History.standalone`、`PersistentHistory`、`PersistentHistory.standalone`、`PersistentHistory.add`。写出输入、输出、调用方和失败分支。
3. 打开 `src/customer_support/settings.py`：搜索并跟踪：`Settings`、`Settings.from_env`。写出输入、输出、调用方和失败分支。
4. 打开 `src/customer_support/thread_store.py`：搜索并跟踪：`Message`、`SQLiteThreadStore`、`SQLiteThreadStore.append`、`SQLiteThreadStore.load`。写出输入、输出、调用方和失败分支。
5. 打开 `tests/test_application.py`：搜索并跟踪：`Assistant`、`Assistant.ask`、`test_product_follow_up_survives_history_object_restart`。写出输入、输出、调用方和失败分支。
6. 打开 `tests/test_thread_store.py`：搜索并跟踪：`test_messages_persist_and_are_isolated`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 60
cd .build\day60\customer-support
...
SQLite 只代表本地单实例验证。把实际运行结果写入 `workbook.md`。
