...
先还原并跑通 Day77，再开始今天。Day78 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。
...
1. 打开 `src/customer_support/acceptance.py`：搜索并跟踪：`accept`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_final_acceptance.py`：搜索并跟踪：`test_every_required_capability_must_pass`。写出输入、输出、调用方和失败分支。
