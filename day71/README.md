...
先还原并跑通 Day70，再开始今天。Day71 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。
...
1. 打开 `src/customer_support/vector_store.py`：搜索并跟踪：`VectorStore`、`VectorStore.upsert`、`VectorStore.delete_source`、`VectorStore.search`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_vector_store_contract.py`：搜索并跟踪：`MemoryStore`、`MemoryStore.upsert`、`MemoryStore.delete_source`、`MemoryStore.search`、`test_contract_requires_tenant_scoped_search_and_delete`。写出输入、输出、调用方和失败分支。
