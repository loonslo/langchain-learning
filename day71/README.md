...
> 今天解决：Chroma 不能代表最终共享存储。
>
> 第一性原则：业务依赖 VectorStore 契约而非具体数据库。

## 1. 与 Day70 的文件衔接

先还原并跑通 Day70，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。

### 今天新增、修改或删除

| 项目相对路径 | 状态 | 为什么今天要看 |
|---|---|---|
| `deployment/pgvector_schema.sql` | 新增 | 项目配置、文档或交付证据 |
| `src/customer_support/vector_store.py` | 新增 | 存储协议 |
| `tests/test_sync_vector_store.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `tests/test_vector_store_contract.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/sync.py` | 修改旧文件（上一版 Day64） | 增量同步计划 |
...
运行 `python tools/day_change_report.py 71` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
scan/plan → apply_plan → VectorStore.upsert/delete_source
```
...
deployment/pgvector_schema.sql  # 本日变更
...
src/customer_support/bootstrap.py  # 继承 Day70
...
src/customer_support/sync.py  # 本日变更
...
src/customer_support/vector_store.py  # 本日变更
...
tests/test_sync_vector_store.py  # 本日变更
tests/test_thread_store.py  # 继承 Day60
tests/test_tickets.py  # 继承 Day59
tests/test_tool_runner.py  # 继承 Day58
tests/test_vector_store_contract.py  # 本日变更
tests/test_workflow.py  # 继承 Day56
```

## 4. 按顺序阅读和动手

1. 打开 `src/customer_support/sync.py`：搜索并跟踪：`SyncPlan`、`scan`、`plan`、`apply_plan`、`SyncingApplication`、`SyncingApplication.handle`、`SyncingApplication.ask`、`SyncingApplication.plan_sync`。写出输入、输出、调用方和失败分支。
2. 打开 `src/customer_support/vector_store.py`：搜索并跟踪：`VectorStore`、`VectorStore.upsert`、`VectorStore.delete_source`、`VectorStore.search`。写出输入、输出、调用方和失败分支。
3. 打开 `tests/test_sync_vector_store.py`：搜索并跟踪：`Store`、`Store.upsert`、`Store.delete_source`、`Store.search`、`test_sync_plan_reaches_the_vector_store_contract`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_vector_store_contract.py`：搜索并跟踪：`MemoryStore`、`MemoryStore.upsert`、`MemoryStore.delete_source`、`MemoryStore.search`、`test_contract_requires_tenant_scoped_search_and_delete`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 71
cd .build\day71\customer-support
...
schema 存在不等于已完成远端迁移。把实际运行结果写入 `workbook.md`。
