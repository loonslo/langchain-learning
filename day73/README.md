...
> 今天解决：bad case 需要进入可审查改进队列。
>
> 第一性原则：负反馈不能自动改生产 Prompt。

## 1. 与 Day72 的文件衔接

先还原并跑通 Day72，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。
...
| `tests/test_feedback.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `tests/test_feedback_api.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/runtime.py` | 修改旧文件（上一版 Day64） | 真实依赖与最终 API 组合入口 |
...
| `src/customer_support/bootstrap.py` | 继承 Day70，今天仍被变更代码调用 | 创建真实依赖并接入正式主链 |
...
运行 `python tools/day_change_report.py 73` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
同一 FastAPI → /chat、/knowledge/sync-plan、/feedback → FeedbackStore
```
...
src/customer_support/feedback.py  # 本日变更
...
tests/test_feedback.py  # 本日变更
tests/test_feedback_api.py  # 本日变更
...
1. 打开 `src/customer_support/runtime.py`：搜索并跟踪：`create_runtime_api`。写出输入、输出、调用方和失败分支。
2. 打开 `src/customer_support/feedback.py`：搜索并跟踪：`Feedback`、`FeedbackStore`、`FeedbackStore.add`、`FeedbackStore.review_queue`、`FeedbackRequest`、`attach_feedback_routes`。写出输入、输出、调用方和失败分支。
3. 打开 `tests/test_feedback.py`：搜索并跟踪：`test_only_negative_unreviewed_feedback_enters_queue`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_feedback_api.py`：搜索并跟踪：`test_feedback_enters_the_review_store_through_the_product_api`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 73
cd .build\day73\customer-support
...
反馈可能有偏差且需人工复核。把实际运行结果写入 `workbook.md`。
