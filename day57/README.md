...
> 今天解决：知识库不能回答我的订单状态。
>
> 第一性原则：工具读取业务数据前必须验证资源归属。

## 1. 与 Day56 的文件衔接

先还原并跑通 Day56，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。
...
| `tests/test_orders.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/application.py` | 修改旧文件（上一版 Day55） | 统一业务编排 |
| `src/customer_support/bootstrap.py` | 修改旧文件（上一版 Day56） | 创建真实依赖并接入正式主链 |
| `tests/test_application.py` | 修改旧文件（上一版 Day55） | 验证今天新增能力的正向和失败路径 |
...
运行 `python tools/day_change_report.py 57` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
app → application.handle → 订单归属查询或 RAG 问答
```
...
src/customer_support/orders.py  # 本日变更
...
3. 打开 `src/customer_support/orders.py`：搜索并跟踪：`OrderNotFound`、`ForbiddenOrder`、`Order`、`OrderRepository`、`OrderRepository.get_for_user`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_application.py`：搜索并跟踪：`Assistant`、`Assistant.ask`、`test_order_query_is_reachable_from_the_product_application`。写出输入、输出、调用方和失败分支。
5. 打开 `tests/test_orders.py`：搜索并跟踪：`test_order_query_enforces_ownership`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 57
cd .build\day57\customer-support
...
当前只有只读内存订单仓库。把实际运行结果写入 `workbook.md`。
