...
先还原并跑通 Day56，再开始今天。Day57 目录只保存今天新增或修改的完整文件；下方结构图中的“继承 DayNN”文件今天无需重复阅读。
...
1. 打开 `src/customer_support/orders.py`：搜索并跟踪：`OrderNotFound`、`ForbiddenOrder`、`Order`、`OrderRepository`、`OrderRepository.get_for_user`。写出输入、输出、调用方和失败分支。
2. 打开 `tests/test_orders.py`：搜索并跟踪：`test_order_query_enforces_ownership`。写出输入、输出、调用方和失败分支。
