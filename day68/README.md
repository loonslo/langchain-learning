...
> 今天解决：重复问答浪费模型调用。
>
> 第一性原则：缓存键必须包含租户和知识版本且有 TTL。

## 1. 与 Day67 的文件衔接

先还原并跑通 Day67，再开始今天。不要只看新增文件：先确认今天修改了哪些旧文件，再沿“关键继承文件”检查新能力是否真的进入已有产品链。
...
| `tests/test_cache.py` | 新增 | 验证今天新增能力的正向和失败路径 |
| `src/customer_support/bootstrap.py` | 修改旧文件（上一版 Day67） | 创建真实依赖并接入正式主链 |
| `tests/test_application.py` | 修改旧文件（上一版 Day67） | 验证今天新增能力的正向和失败路径 |
...
运行 `python tools/day_change_report.py 68` 可查看全部“继承未改”文件。

## 2. 今天结束后的真实调用链

```text
API/CLI → ObservedApplication → CachedApplication → 业务链
```
...
src/customer_support/cache.py  # 本日变更
...
2. 打开 `src/customer_support/cache.py`：搜索并跟踪：`AnswerCache`、`AnswerCache.key`、`AnswerCache.put`、`AnswerCache.get`、`CachedApplication`、`CachedApplication.handle`、`CachedApplication.ask`。写出输入、输出、调用方和失败分支。
3. 打开 `tests/test_application.py`：搜索并跟踪：`Product`、`Product.handle`、`test_cache_is_on_the_product_path_and_order_queries_bypass_it`。写出输入、输出、调用方和失败分支。
4. 打开 `tests/test_cache.py`：搜索并跟踪：`test_cache_is_scoped_and_expires`。写出输入、输出、调用方和失败分支。
...
.\.venv\Scripts\python.exe tools\materialize_day.py 68
...
尚未实现多实例和击穿保护。把实际运行结果写入 `workbook.md`。
